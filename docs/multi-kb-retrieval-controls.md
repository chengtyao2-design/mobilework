...
普通知识库问答直接调用统一 `retrieve`，由后端内部完成目录路由；不再预先调用 `list_knowledge_bases` 或 `route_knowledge_bases`，避免一次多余的模型往返。只有用户明确检查知识库清单或调试路由时才调用这两个工具。
...
无进展检测在 15 秒提示当前是否已取得资料。首次模型响应及工具完成后的回答生成最多等待 45 秒；只有检索工具已经开始运行时才放宽高级档位，Reasoning 为 75 秒，Research 为 135 秒，自定义高级检索预算则使用“检索 deadline + 45 秒回答缓冲”。已有检索时可以复用证据；首次模型响应尚未返回时重新处理原问题。正常流式输出或工具进展会刷新计时，模型重试状态不会取消检测。修改后需退出并重新启动 Mobilework 以加载新版 TUI 插件。

检索后端默认把轻量生命周期日志写入 `.mobilework-state/logs/retrieval.log`。日志记录工具、路由、各知识库开始/完成/失败、耗时和结果数，不记录 Query 正文与证据正文；可用 `MOBILEWORK_LOG_LEVEL` 调整等级。
