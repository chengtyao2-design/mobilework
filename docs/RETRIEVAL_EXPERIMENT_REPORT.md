...
> 状态：2026-09-08 实验已收口。本文与最终 Excel 基于 1,802 条真实尝试：1,244 条当前有效记录、558 条已被成功或更新尝试替代的审计记录；当前有效记录中含 46 条失败，其中 8 条为明确隔离的历史 Skill 快照。失败不补零，也不用模拟值替代。
...
2. 加入口语术语扩展、语义优先加权融合和低置信图扩展抑制后，模糊问法的 Keyword Hit@5 从优化前 33.3% 提升到 83.3%，Vector + Keyword 从 50.0% 提升到 100%，三通道从 66.7% 提升到 100%；三通道模糊 MRR 从 38.9% 提升到 91.7%。标准问法指标保持不变。自动路由与全库检索均为 87.5% Hit@5，但自动路由 p95 为 18.72 秒，较全库 28.27 秒低 33.8%。
...
优化实现保留原 Query 的实体、数字与否定表达，仅向 Keyword、Graph 和路由补充可审计的规范术语；Vector 仍使用用户原始 Query。口语扩展生效时，融合降低弱词法/图信号权重，并取消强制占用 Top-5 的图邻居槽位。最终 144 条回归均成功。由于只有 6 个独立问题，重复运行不增加统计样本量，本结果仍属于项目级小样本。
...
| 实验 | 对照 | 主要结果 | 可支持的结论 |
|---|---|---|---|
| 模糊 Query | 标准 / 口语 | Keyword Hit@5 66.7%→83.3%；Vector 100%→100%，MRR 100%→88.9%；Vector+Keyword 100%→100%，模糊 MRR 80.6%；三通道 83.3%→100%，模糊 MRR 91.7% | 规范术语扩展恢复词法与路由召回；语义优先融合避免弱通道把正确向量结果挤出 Top-5。结论仅限 6 个独立问题 |
